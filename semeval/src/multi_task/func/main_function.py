...
from src.multi_task.model.model import RobertaForMultiTaskLearning
...
def train(config):
    ...
    roberta_classification = RobertaForMultiTaskLearning.from_pretrained('roberta-large', config=roberta_config)
...
def test(config):
    ...
    roberta_classification = RobertaForMultiTaskLearning.from_pretrained(
        os.path.join(config.output_dir, "{}_checkpoint_{}".format(config.PMmodel, config.checkpoint)),
    ...
    test_accuracy, total_predicts = do_evaluate(model=roberta_classification, test_dataloader=dev_dataloader)
    print("test_accuracy : {}\n".format(round(test_accuracy, 4)))
