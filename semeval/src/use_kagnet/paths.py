...
def find_paths_sa_concept_pair(source: str, target: str, ifprint=False):
    """
    ...
    # source랑 target은 lose_weight같은 식으로 들어가야함
    source = source.replace(' ', '_')
    target = target.replace(' ', '_')

    if (source not in id2concept) or (target not in id2concept):
        return
...
def find_paths_sa_pair(sa_pair):
    acs, qcs = sa_pair
    pfr_sa = []
    for ac in acs:
        for qc in qcs:
            pf_res = find_paths_sa_concept_pair(qc, ac)
            pfr_sa.append({"ac": ac, "qc": qc, "pf_res": pf_res})
    return pfr_sa


# grounded_path : 멀티태스크로 컨셉들 뽑은거로 대체
def find_paths(grounded_path, cpnet_vocab_path, cpnet_graph_path, output_path, num_processes=1, random_state=0):
    ...
    for idx, item in enumerate(data):
        if len(item['answer_span_concept']):
            choice_data.append([item["answer_span_concept"], item["sentence_span_concept"]])
        else:
            if len(item["answer_instance"].split(" "))>1:
                choice_data.append([item["answer_instance"].split(" ")[1], item["sentence_span_concept"]])
            else:
                choice_data.append([item["answer_instance"], item["sentence_span_concept"]])
    data = choice_data

    with Pool(num_processes) as p, open(output_path, 'w', encoding='utf-8') as fout:
        for pfr_qa in tqdm(p.imap(find_paths_sa_pair, data), total=len(data)):
...
def score_paths(raw_paths_path, concept_emb_path, rel_emb_path, cpnet_vocab_path, output_path, num_processes=1, method='triple_cls'):
    ...
    for statement in tqdm(data, total=len(data)):
        with open(output_path, 'a', encoding='utf-8') as fout:
            fout.write(json.dumps(score_qa_pairs(statement))+'\n')
